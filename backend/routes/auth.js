...
router.post('/register', [
  body('username').trim().isLength({ min: 3, max: 20 }).withMessage('Логин должен быть от 3 до 20 символов').matches(/^[a-zA-Z0-9_]+$/).withMessage('Логин может содержать только буквы, цифры и _'),
  ...
  try {
    ...
    const isAdmin = username.toLowerCase() === INITIAL_ADMIN.toLowerCase();
...
router.post('/login', [
  ...
  try {
    ...
    if (user.username.toLowerCase() === INITIAL_ADMIN.toLowerCase() && user.role !== 'admin') {
      user.role = 'admin';
    }
